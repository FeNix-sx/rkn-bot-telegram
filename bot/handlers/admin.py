...
async def admin_bound(cb: CallbackQuery, settings: Settings, xui_api: XUIAPI):
    """Показывает привязанных юзеров (дублирует админскую статистику)."""
    if not await _is_admin(cb.from_user.id, settings, None):
        return await cb.answer("🚫", show_alert=True)
    await cb.answer()
    # Встроенная логика, чтобы не зависеть от импортов из stats.py
    try:
        ...
        text = "📊 Привязанные:\n\n" + "\n\n──────────────\n\n".join(blocks[:5])
        await cb.message.answer(text, reply_markup=back_admin())
...
async def admin_unbound(cb: CallbackQuery, settings: Settings, users_repo: UsersRepository, xui_api: XUIAPI):
    ...
    try:
        ...
        if not unbound:
            return await cb.message.answer("✅ Все клиенты из панели привязаны.", reply_markup=back_admin())
        total = len(unbound)
        UNBOUND_PAGE_SIZE = 7
        total_pages = (total + UNBOUND_PAGE_SIZE - 1) // UNBOUND_PAGE_SIZE
        page = max(0, min(page, total_pages - 1))
        current = unbound[page * UNBOUND_PAGE_SIZE : (page + 1) * UNBOUND_PAGE_SIZE]
...
async def bind_select_client(cb: CallbackQuery, settings: Settings, users_repo: UsersRepository):
    ...
    kb = [[InlineKeyboardButton(text=f"👤 {u['username'] or u['first_name'] or f'ID:{u['tg_id']}'} ({u['tg_id']})", callback_data=f"bind:do_tg:{u['tg_id']}:{email}")] for u in tg_users[:5]]
    ...
    try:
        ...
        async with aiosqlite.connect(users_repo.db_path) as db:
            cur = await db.execute(
                "UPDATE users SET xui_email=?, xui_uuid=?, username=COALESCE(NULLIF(?, ''), username), updated_at=? WHERE tg_id=?",
                (email, uuid, "", now, tg_id)
            )
            if cur.rowcount == 0:
                await db.execute(
                    "INSERT INTO users (tg_id, username, xui_email, xui_uuid, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (tg_id, "", email, uuid, 'manual', now, now)
                )
            await db.commit()
    ...
    try:
        async with aiosqlite.connect(users_repo.db_path) as db:
            ...
            async with db.execute("""
                ...
                FROM users
                WHERE xui_email IS NULL OR xui_email = ''
                ORDER BY created_at DESC
            """) as cur:
                unknown = await cur.fetchall()
        if not unknown:
            return await cb.message.answer("✅ Все пользователи из бота привязаны к панели.", reply_markup=back_admin())
        kb = [[InlineKeyboardButton(
            text=f"👤 {u['username'] or u['first_name'] or f'ID:{u['tg_id']}'} ({u['tg_id']})",
            callback_data=f"bind:unknown:{u['tg_id']}"
        )] for u in unknown[:10]]
        kb.append([InlineKeyboardButton(text="🔙 К управлению", callback_data="admin:main")])
...
async def admin_unknown_select(cb: CallbackQuery, settings: Settings, users_repo: UsersRepository, xui_api: XUIAPI):
    ...
    try:
        ...
        if not unbound_clients:
            return await cb.message.answer("✅ В панели нет свободных клиентов для привязки.", reply_markup=back_admin())
...
async def bind_unknown_do(cb: CallbackQuery, settings: Settings, users_repo: UsersRepository, xui_api: XUIAPI):
    ...
    try:
        ...
        for ib in inbounds:
            ...
            if uuid: break
        if not uuid:
            return await cb.message.answer("❌ UUID не найден в панели.", reply_markup=back_admin())
        now = utc_now_iso()
        async with aiosqlite.connect(users_repo.db_path) as db:
            cur = await db.execute(
                "UPDATE users SET xui_email=?, xui_uuid=?, status='active', updated_at=? WHERE tg_id=?",
                (target_email, uuid, now, tg_id)
            )
            if cur.rowcount == 0:
                return await cb.message.answer(f"❌ Юзер {tg_id} не найден в БД.", reply_markup=back_admin())
            await db.commit()
