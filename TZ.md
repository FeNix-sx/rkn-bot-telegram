...
> **Актуальный чеклист «сделано / нет»:** раздел [§3.1 Статус реализации](#31-статус-реализации-по-коду-репозитория) ниже. Пункты §3 оставлены как исходное ТЗ.
...
### 3.1 Статус реализации (по коду репозитория)

*Обновлено по состоянию ветки разработки. Легенда: **✅** готово · **⚠️** есть, но не как в §3/§2 · **⬜** нет в коде · **🔧** настройка/сервер, не код.*

| Этап | Статус | Что в коде |
|------|--------|------------|
| **0. Фундамент** | ⚠️✅ | `core/xui_api.py`: логин, `get_inbounds`, поиск inbound/email, `add_client`, вкл/выкл клиента, срок, `limitIp`, сборка **vless://** (`build_vless_share_uri`, в т.ч. Reality/`spx`). Подписка в смысле ТЗ: пользователю отдаётся готовая ссылка из панели/бота, не поднятие своего `/sub`. **Схема БД** — `db/schema.sql`: проще, чем §2 (нет `traffic_limit_bytes`, `limit_ip` в users как в ТЗ; `payments` — укороченная таблица). **Тесты API:** в репозитории почти нет `tests/` (не как в §0). **Dockerfile/compose** в репозитории не найдены. |
| **1. Пользовательский флоу** | ⚠️ | `/start`, кнопка триала, согласование новых пользователей админом; `/trial` (через одобрение), выдача клиента в X-UI, `trial_*` email; **«📊 Мой статус»** / `/status` — карточка трафика и подписки; **«🔗 Моя ссылка»** / `/link` — vless. **⬜** `/buy`, тарифы устройств, `payment_pending`, реквизиты и `order_id` — не реализованы. |
| **2. Планировщик** | ⬜ | `APScheduler`, `check_trials()`, предупреждения 48h/24h и автоотключение по `trial_end` в коде не найдены. |
| **3. Платежи** | ⬜ | `PaymentsRepository` — заготовка; полного флоу «я оплатил / скрин / модерация админом» как в ТЗ нет. |
| **4. Админка** | ⚠️ | Не `/admin` из ТЗ, а **reply «⚙️ Управление»** + inline: **Пользователи** (список с X-UI, карточка, продление, IP limit, вкл/выкл, заглушки удаления и т.д.), **Нет аккаунта ТГ** (клиенты только в панели — полноценная карточка + привязка к TG + удаление), **Нет клиента VPN**, добавление клиента, динамические админы. **📈 Статистика** у админа: меню → **Пользователи** / **Клиенты в панели** (сводка по трафику и параметрам). **⬜** отдельный «Поиск @username», «журнал платежей», «Сброс трафика» как в ТЗ. |
| **5. Docker** | ⬜ | Нет `Dockerfile` / `docker-compose` в дереве проекта (по текущему снимку). |
| **6. Деплой** | 🔧 | Процедуры на сервере — вне репозитория. |

**DoD (§4) — кратко:** триал и ссылки — по факту доводились под клиентов; автоотключение по расписанию, оплата end-to-end, Docker, покрытие тестами >70%, образ <150MB — **в основном не закрыты** текущим кодом.
